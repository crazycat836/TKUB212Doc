---
title: 修改印表機 IP (M201dw)
description: HP LaserJet M201dw 印表機的 IP 修改流程，界面與 1606dn 略有差異。
order: 6
group: printer
---
...
8. 此時會要你輸入密碼，輸入完後就可以順利進到印表機設定畫面。假如沒跳出要輸入密碼，一定要先到「系統」頁面底下，「管理」那邊做修改（修改畫面如下圖）。

   ![設定密碼](../../assets/images/newprinter4.jpg)

9. 下圖為成功進入的初始畫面。

   ![初始畫面](../../assets/images/newprinter1.jpg)
...
10. 輸入完密碼或是密碼設定後，就可以進到「網路」頁面底下選擇「IPv4 組態」這邊去修改 IP，IP 偏好位置方法使用手動。

    ![IPv4 組態](../../assets/images/newprinter3.jpg)
    ...
    改完 IP 後，點選左邊的 SNMP，選擇「啟用 SNMP v1/v2 唯讀存取...」（跟圖中的選項相同）。

    ![SNMP 設定](../../assets/images/newprinter2.jpg)

11. 送回實習室前，務必印一張測試頁，確認設定無誤。
